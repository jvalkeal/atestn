...
import { Activity, DEFAULT_GENERATE_CHECKSUM_CONFIG, GenerateChecksum } from '../src/interfaces';
...
describe('nexus-sync tests', () => {
  ...
  it('Missing checksum config should use default', async () => {
    ...
    expect(generateChecksumFilesSpy).toHaveBeenCalledWith('nexus', DEFAULT_GENERATE_CHECKSUM_CONFIG);
  }, 100000);

  it('Defined checksum config should override default', async () => {
    const checksumConfig: GenerateChecksum[] = [{ type: 'foo', extension: 'foo' }];
    ...
    expect(waitRepoStateSpy).not.toHaveBeenCalled();
    // expect(generatePgpFilesSpy).not.toHaveBeenCalled();
    expect(generateChecksumFilesSpy).toHaveBeenCalledWith('nexus', checksumConfig);
  }, 100000);
});
